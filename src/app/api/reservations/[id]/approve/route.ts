...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  try {
    // 공통 서비스인 updateStatus를 호출하여 토큰 검증 및 예약 승인 처리
    revalidatePath("/calendar");
    revalidatePath("/book");
    const reservation = await updateStatus(id, "approved", {
      source: "email",
      token,
      reviewedBy: "system-email-action", // 자동 이메일 처리를 식별할 수 있는 식별자 주입
    });
    ...
    // 프리미엄 디자인이 적용된 승인 완료 HTML 페이지 반환
    const html = `
      <!DOCTYPE html>
      <html lang="ko">
      <head>
      ...
      <body>
        <div class="card">
          ...
          <h1>예약 승인 완료</h1>
          <p class="description">해당 예약 신청이 성공적으로 승인되었습니다.<br>예약자에게 확정 안내 메일이 발송되었습니다.</p>
    ...
    let status = 500;
    let title = "예약 처리 실패";
    let message = "예약을 승인하는 도중 오류가 발생했습니다.";
    ...
    if (error instanceof ReservationServiceError) {
      ...
      if (error.code === "TOKEN_INVALID" || error.code === "TOKEN_REQUIRED") {
        ...
        title = "날짜 중복";
        message =
          "선택하신 일정 범위에 다른 예약이 이미 확정되었거나 대기 중이므로 승인할 수 없습니다.";
      }
